...
namespace tianli {
  ...
  inline void tianli_widget_super::afterClose()
  {
    ...
    {
      QProcess process;
      process.startDetached(QString::fromWCharArray(L"cmd.exe"), QStringList() << "/c" << "cloneInstaller.bat");
    }
    //只有安装器在Temp目录，才允许自我删除，以避免误删
    fs::path pt1 = fs::absolute(fs::path(L".."));
    fs::path pt2 = tianliUtils::envPath2AbsolutePath(L"%TEMP%");
    if (fs::equivalent(fs::path(L"..\\"), tianliUtils::envPath2AbsolutePath(L"%TEMP%")))
    {
      QFile removeSelfBat(QString::fromWCharArray(L"removeSelf.bat"));
      ...
      QProcess process;
      process.startDetached(QString::fromWCharArray(L"cmd.exe"), QStringList() << "/c" << "removeSelf.bat");
      return;
    }
  }
  ...
  void tianli_widget_super::initTimeLine()
  {
    ...
    timeLineLabel_1->findChild<QLabel*>("label_Title")->setText(QString::fromWCharArray(L"复制文件"));
    ...
    timeLineLabel_2->findChild<QLabel*>("label_Title")->setText(QString::fromWCharArray(L"写入注册表"));
    ...
    timeLineLabel_3->findChild<QLabel*>("label_Title")->setText(QString::fromWCharArray(L"创建快捷方式"));
    ...
    timeLineLabel_4->findChild<QLabel*>("label_Title")->setText(QString::fromWCharArray(L"清理安装缓存"));
  ...
  bool tianli_widget_super::eventFilter(QObject* object, QEvent* event)
  {
    ...
    if (event->type() == QEvent::Enter)
    {
      ...
      mainShadowAnimation->setEndValue(L"#FF1C1C");
      mainShadowAnimation->start();
    }
    if (event->type() == QEvent::Leave)
    {
      mainShadowAnimation->stop();
      mainShadowAnimation->setEndValue(L"#DDDDDD");
      mainShadowAnimation->start();
    }
  ...
  void tianli_widget_super::pushButton_Fast()
  {
    ...
    if (tianliUtils::checkPathIsAvilable(folderPathStr, outPathStr))
    {
      ...
      {
        QMessageBox::warning(this, QString::fromStdWString(L"路径无效"), QString::fromStdWString(L"请选择空白文件夹或者旧版“空荧酒馆原神地图”安装路径"));
        return;
      }
  ...
  {
    QString folderPathStr = QFileDialog::getExistingDirectory(this, QString::fromStdWString(L"请选择需要安装的文件夹"), pathLineEdit->text());
    if (folderPathStr == "")
      return;
  ...
  void tianli_widget_super::pushButton_Cancel()
  {
    ...
    errorInfoLabel->setText(QString::fromStdWString(L"用户终止了安装操作"));
  }

  void tianli_widget_super::pushButton_Finished_Run()
  {
    QProcess process;
    std::wstring processPath = std::format(L"\"{0}\\{1}\"", std::wstring(ui->lineEdit_installPath->text().toStdWString()), config::installInfo.exePath);
    process.setProgram(QString::fromStdWString(processPath));
    process.setWorkingDirectory(QString::fromStdWString(std::wstring(ui->lineEdit_installPath->text().toStdWString())));
